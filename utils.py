...
def get_opp_list():
    """
    Used in L4 to get a flat list of opps
    """
    opps = get_opps()
    opp_list = []
    for opp in opps.keys():
        opp_list.append(opp)

    return opp_list


def get_opps_and_category():
    """
    Used in L5
    """
    opps = get_opps()
    opps_by_category = defaultdict(list)
    for opp_name, opp_info in opps.items():
        category = opp_info.get('category')
        if category:
            opps_by_category[category].append(opp_info.get('nonprofit_organization'))

    return dict(opps_by_category)


def get_opps():
    with open(opps_file, 'r') as file:
        opps = json.load(file)
    return opps


def find_category_and_opp(user_input, opps_and_category):
    system_message = f"""
  You will be provided with customer service queries. \
    ...
    Allowed opportunities: {opps_and_category}
...
def find_category_and_opp_only(user_input):
    system_message = f"""
    You will be provided with customer service queries. \
    The customer service query will be delimited with {delimiter} characters.
    Output a python list of json objects, where each object has the following format:
...
def get_opps_from_query(user_msg):
    """
    Code from L5, used in L8
    """
    opps_and_category = get_opps_and_category()
    system_message = f"""
    You will be provided with customer service queries. \
    The customer service query will be delimited with {delimiter} characters.
    Output a python list of json objects, where each object has the following format:
    ...
    category_and_opp_response = get_completion_from_messages(messages)

    return category_and_opp_response


# opp look up (either by category or by opp within category)
def get_opp_by_name(name):
    opps = get_opps()
    return opps.get(name, None)


def get_opps_by_category(category):
    opps = get_opps()
    return [opp for opp in opps.values() if opp["category"] == category]


def get_mentioned_opp_info(data_list):
    """
    Used in L5 and L6
    """
    opp_info_l = []
    ...
    for data in data_list:
        try:
            if "opps" in data:
                opps_list = data["opportunities"]
                for opp_name in opps_list:
                    opp = get_opp_by_name(opp_name)
                    if opp:
                        opp_info_l.append(opp)
                    else:
                        print(f"Error: Opportunity '{opp_name}' not found")
            elif "category" in data:
                category_name = data["category"]
                category_opps = get_opps_by_category(category_name)
    ...
    for data in data_list:
        try:
            if "opportunity" in data:
                opps_list = data["opportunity"]
                for opp_name in opps_list:
                    opp = get_opp_by_name(opp_name)
                    if opp:
                        output_string += json.dumps(opp, indent=4) + "\n"
                    else:
                        print(f"Error: opportunity '{opp_name}' not found")
            elif "category" in data:
                category_name = data["category"]
...
# Example usage:
# opp_information_for_user_message_1 = generate_output_string(category_and_opp_list)
# print(opp_information_for_user_message_1)

def answer_user_msg(user_msg, opp_info):
    """
    ...
    """
    # user_msg = f"""
    # tell me about the Youth Empowerment League . Also, tell me about your Environmental Cleanup"""
    messages = [
        {'role': 'system', 'content': system_message},
        {'role': 'user', 'content': f"{delimiter}{user_msg}{delimiter}"},
...
def create_opps():
    """
        Create opps dictionary and save it to a file named opps.json
    """
    # opp information
    # fun fact: all these opps are fake and were generated by a language model
    opps = {
        "Helping Hands Foundation":
            {
